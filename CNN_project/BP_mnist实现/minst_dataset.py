...
class Mnist_dataset:
    def __init__(self, path, train=True):
        ...
        self.labels=self.one_hot(self.labels,10)
    ...
    def load_labels(path):
        with open(path, 'rb') as f:
            labels = f.read()
        labels = np.asarray(bytearray(labels[8:]), dtype=np.float32).reshape(-1,1)
        return labels

    @staticmethod
    def one_hot(labels, classifi):
        classifi = np.arange(classifi)
        return (labels == classifi).astype(np.float32)
